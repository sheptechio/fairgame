...
class Amazon:
    ...
    def is_logged_in(self):
        try:
            text = self.driver.find_element("id", "nav-link-accountList").text
    ...
    def login(self):
        ...
        while True:
            try:
                email_field = self.driver.find_element("xpath", '//*[@id="ap_email"]')
                break
            except sel_exceptions.NoSuchElementException:
                try:
                    password_field = self.driver.find_element("xpath", 
                        '//*[@id="ap_password"]'
                    )
                    break
                except sel_exceptions.NoSuchElementException:
        ...
        try:
            self.driver.find_element("xpath", '//*[@name="rememberMe"]').click()
        except sel_exceptions.NoSuchElementException:
        ...
        if password_field:
            ...
            # check for captcha
            try:
                captcha_entry = self.driver.find_element("xpath", 
    ...
    def check_stock(self, asin, reserve_min, reserve_max, retry=0):
        ...
        while True:
            ...
            test = None
            try:
                test = self.driver.find_element("xpath", 
    ...
    def buy_it_now(self, offering_id, max_atc_retries=DEFAULT_MAX_ATC_TRIES):
        ...
        while not successful:
            ...
            if self.driver.title not in amazon_config["CHECKOUT_TITLES"]:
                ...
                continue
            try:
                place_order_button = self.driver.find_element("xpath", 
    ...
    def handle_checkout(self, test):
        ...
        while True:
            try:
                button = self.driver.find_element("xpath", self.button_xpaths[0])
            except sel_exceptions.NoSuchElementException:
                if self.shipping_bypass:
                    try:
                        button = self.get_amazon_element(key="ADDRESS_SELECT")
    ...
    def handle_captcha(self, check_presence=True):
        ...
        current_page = self.driver.title
        try:
            if not check_presence or self.driver.find_element("xpath", 
                '//form[contains(@action,"validateCaptcha")]'
            ):
                try:
                    ...
                    # Solved (!?)
                        ...
                        try:
                            captcha_field = self.driver.find_element("xpath", 
    ...
    def wait_for_page_content_change(self, timeout=5):
        ...
        old_page = self.driver.find_element("tag name", "html")
        yield
        try:
    ...
    def get_page(self, url):
        ...
        current_page = []
        try:
            check_cart_element = self.driver.find_element("xpath", 
                '//*[@id="nav-cart"]'
            )
        except sel_exceptions.NoSuchElementException:
            current_page = self.driver.title
